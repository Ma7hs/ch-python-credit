...
def verificar_dados(i):
      ...
      if tipo_documento == "CPF":
        while len(numero_documento) <= 10 or len(numero_documento) > 11:
          print('Validação de CPF incorreta')
          numero_documento = input(f'Digite o número do {tipo_documento}: ')
      elif tipo_documento == "CNPJ":
        while len(numero_documento) <= 13 or len(numero_documento) > 15:
          print('Validação de CNJP incorreta')
          numero_documento = input(f'Digite o número do {tipo_documento}: ')
...
for i in range(1, 6):
    ...
    print(f"Solicitante {i} cadastrado dentro do sistema")
    print('-----------------------------------------------')
